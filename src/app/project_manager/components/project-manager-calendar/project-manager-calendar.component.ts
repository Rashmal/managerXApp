...
import { CalendarTaskDetails } from '../../../admin/core/calendarTaskDetails';
...
@Component({
  selector: 'app-project-manager-calendar',
  templateUrl: './project-manager-calendar.component.html',
  styleUrl: './project-manager-calendar.component.scss',
  providers: [DatePipe],
  standalone: false
})
export class ProjectManagerCalendarComponent {
  ...
  calendarOptions: CalendarOptions = {
    ...
    eventContent: (arg) => {
      ...
      `;
        ...
        <div class="event-stage" style='font-size:7pt'>${subStageCategoryDetailsName} | ${stageName}</div>
        <div style='display:flex;align-items:center'>
          <input disabled type="checkbox" id="checkbox-${taskId}" ${taskcheck ? 'checked' : ''}>
          <b style='padding-left:7px' class="${statusCode}">${event.title}</b>
        </div>
      `;
      }
